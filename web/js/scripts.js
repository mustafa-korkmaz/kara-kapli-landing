...
(function ($) {
    ...
    $("#requestForm").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            ...
            rformError();
            rsubmitMSG(false, "Lütfen formu eksiksiz doldurun!");
        } else {
    ...
    $("#contactForm").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            // handle the invalid form...
            cformError();
            csubmitMSG(false, "Lütfen formu eksiksiz doldurun!");
        } else {
            // everything looks good!
            event.preventDefault();
            csubmitForm();
    ...
    function cformSuccess() {
        $("#contactForm")[0].reset();
        csubmitMSG(true, "Mesajınızı aldık, en kısa sürede size geri dönüş yapacağız!");
        $("input").removeClass('notEmpty'); // resets the field label after submission
    ...
    $("#privacyForm").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            // handle the invalid form...
            pformError();
            psubmitMSG(false, "Lütfen formu eksiksiz doldurun!");
        } else {
            // everything looks good!
            event.preventDefault();
            psubmitForm();
